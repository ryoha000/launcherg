...
describe('dLsite Extractor', () => {
  ...
  describe('ゲーム情報の抽出', () => {
    it('dLsiteのHTMLから正しくゲーム情報を抽出できること', () => {
      ...
      const _expectedStoreIds = new Set(['VJ01004076', 'VJ01000588', 'RJ358346', 'RJ01225565', 'RJ01144692']) // ファイル名から抽出される正しいID
      ...
      gameContainers.forEach((container) => {
        ...
        if (thumbnailElement) {
          ...
          if (thumbnailMatch) {
            ...
            const rjMatch = thumbnailUrl.match(/\/(RJ|VJ|BJ)(\d+)/)
            if (rjMatch) {
    ...
    it('rJコードが正しく抽出されること', () => {
      ...
      testUrls.forEach((url, index) => {
        ...
        const matches = url.match(/\/(RJ|VJ|BJ)(\d+)/g)
        expect(matches).toBeTruthy()
        if (matches && matches.length > 0) {
          // 最後のマッチを使用（ファイル名部分）
          const lastMatch = matches[matches.length - 1]
          const storeIdMatch = lastMatch.match(/\/(RJ|VJ|BJ)(\d+)/)
          if (storeIdMatch) {
      ...
      function normalizeDLsiteDate(dateStr: string): string {
        try {
          ...
          if (yearMatch && monthMatch && dayMatch) {
            ...
            const month = Number.parseInt(monthMatch[1]) - 1 // Dateオブジェクトは0ベース
