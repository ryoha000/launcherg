...
export class PopupController {
  ...
  private async loadStoredData(): Promise<void> {
    try {
      // 設定を読み込み
      const stored = await chrome.storage.local.get(['extension_config', 'sync_logs'])
  ...
  private updateDetailedStats(): void {
    // メイン統計と同じ値を詳細統計にも反映
    const totalSynced = document.getElementById('total-synced')?.textContent || '0'
    const successCount = document.getElementById('success-count')?.textContent || '0'
    const errorCount = document.getElementById('error-count')?.textContent || '0'

    const totalSyncedDetailed = document.getElementById('total-synced-detailed')
  ...
  private async sendDebugJson(): Promise<void> {
    ...
    if (!jsonText) {
      this.displayDebugResponse({ error: 'JSONメッセージが入力されていません' }, true)
      return
    }
  ...
  private displayDebugResponse(response: any, isError: boolean): void {
    ...
    responseDiv.innerHTML = `
      <div class="debug-response-header ${isError ? 'error' : 'success'}">
        ...
        <span class="response-time">${new Date().toLocaleTimeString('ja-JP')}</span>
      </div>
      <pre class="debug-response-content"><code>${this.escapeHtml(formattedJson)}</code></pre>
    `
  }
  ...
  private updateLogDisplay(): void {
    ...
    if (this.logs.length === 0) {
      logContainer.innerHTML = '<p class="text-center" style="color: #999; padding: 20px;">ログはありません</p>'
      return
    }
