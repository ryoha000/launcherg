<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      packList = packs
      packIds = new Set(packs.map(p => p.storeId))
    } finally {
      loading = false
    }
  }

  async function togglePack(storeId: string, checked: boolean) {
    if (checked) await invoke('dmm_pack_add', { storeId })
    else await invoke('dmm_pack_remove', { storeId })
    await load()
  }
  ...
  {:else}
    ...
    <div class="mt-6 space-y-2">
      ...
      <ul class="space-y-1">
        {#each packList as p}
          <li class="flex items-center gap-2">
            <code class="px-1 py-0.5 bg-gray-100 rounded">{p.storeId}</code>
            <button class="px-2 py-1 border rounded" on:click={() => removePack(p.id, p.storeId)}>削除</button>
          </li>
        {/each}
      ...
      <div class="flex items-center gap-2 mt-2">
        <input class="border px-2 py-1 rounded w-64" placeholder="storeId を入力 (例: purple_0028pack)" bind:value={newStoreId} />
        <button class="px-2 py-1 border rounded" on:click={addPackManually}>追加</button>
      </div>
    </div>
  {/if}
</div>
