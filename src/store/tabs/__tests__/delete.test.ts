...
function t(id: number, type: string, workId: number): Tab {
...
describe('tabs/delete: 削除後の遷移・選択の決定', () => {
  ...
  it('存在しないIDは何もしない', () => {
    const tabs = [t(1, 'settings', -1), t(2, 'store-mapped', -1)]
  ...
  it('非選択を削除しても遷移しない（選択は維持）', () => {
    const tabs = [t(1, 'settings', -1), t(2, 'store-mapped', -1), t(3, 'settings', -1)]
  ...
  it('選択中（左端）を削除 → 右へ遷移', () => {
    const tabs = [t(1, 'settings', -1), t(2, 'works', 42)]
    const plan = computeTabDeletionPlan({ tabs, selectedIndex: 0, deleteId: 1, registry: REG })
    expect(plan.shouldDelete).toBe(true)
