...
mod tests {
    ...
    async fn test_get_recorded_events() {
        ...
        assert!(
            events.len() > 0,
            "イベントが記録されませんでした（ProcTailサービスの設定を確認してください）"
        );
    ...
    async fn test_process_not_found_error() {
        ...
        assert!(
            matches!(
                error,
                crate::domain::windows::proctail::ProcTailError::ProcessNotFound(_)
            ) || matches!(
                error,
                crate::domain::windows::proctail::ProcTailError::ServiceError(_)
            )
        );
    }
    ...
    async fn test_tag_not_found_error() {
        ...
        assert!(
            matches!(
                error,
                crate::domain::windows::proctail::ProcTailError::TagNotFound(_)
            ) || matches!(
                error,
                crate::domain::windows::proctail::ProcTailError::ServiceError(_)
            )
