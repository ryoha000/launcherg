...
fn handle_shortcut_のテスト() {
    ...
    for (i, c) in cases.into_iter().enumerate() {
        ...
        match (c.meta, res) {
            ...
            (MetaKind::IconPng { .. }, Ok(SourceDecision::FallbackDefaultAndSkip { .. })) => {}
...
async fn resolve_local_src_pathに実際のファイルを渡してアイコンが一時的にローカルへ保存される() {
    ...
    for c in cases {
        ...
        match result {
            ...
            Err(e) => {
                assert!(false, "{} はエラーではありません: {}", c.name, e);
            }
        }
    }
...
async fn drain_until_empty_成功コピー_only_mark_finished() {
    ...
    let counter = Arc::new(Mutex::new(0));
    {
        ...
        iq.expect_list().returning(move |unfinished, _| {
...
#[tokio::test]
async fn drain_until_empty_skip経路_デフォルト書き出しと_mark_failed() {
    let repos = TestRepositories::default();
    let manager = Arc::new(TestRepositoryManager::new(repos.clone()));
    let tmp = TempDir::new().unwrap();
    let resolver = Arc::new(TestResolver::new(tmp.path().to_string_lossy().to_string()));
...
#[tokio::test]
async fn drain_until_empty_イベントハンドラ_host_log_が書き込まれる() {
    // Arrange
    use domain::repository::mock::{TestRepositories, TestRepositoryManager};
    let repos = TestRepositories::default();
    let manager = std::sync::Arc::new(TestRepositoryManager::new(repos.clone()));
