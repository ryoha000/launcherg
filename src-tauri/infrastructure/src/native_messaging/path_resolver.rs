...
impl NativeHostPathResolver {
                ...
                match subkey.get_value::<String, _>("") {
                    Ok(manifest_path) => {
                        ...
                        match Self::get_executable_path_from_manifest(&manifest_path) {
                            Ok(exe_path) => {
                                log::debug!(
                                    "Chrome executable path resolved: {}",
                                    exe_path.display()
                                );
                                return Ok(exe_path);
                            }
    ...
    fn get_executable_path_from_manifest(
        manifest_path: &str,
        ...
        // ファイル読み取り
        let mut manifest_content = std::fs::read_to_string(manifest_path).map_err(|e| {
            let error = format!(
                "Failed to read manifest file {}: {} (kind: {:?})",
                manifest_path,
                e,
                e.kind()
            );
        ...
        // pathフィールドの取得
        let exe_path_str = manifest
            .get("path")
            .and_then(|p| p.as_str())
            .ok_or_else(|| {
                let error = format!(
                    "No 'path' field in manifest {}. Available fields: {:?}",
                    manifest_path,
                    manifest
                        .as_object()
        ...
        if !exe_path.exists() {
            let error = format!(
                "Executable not found: {} (absolute path: {})",
                exe_path.display(),
                exe_path
                    .canonicalize()
