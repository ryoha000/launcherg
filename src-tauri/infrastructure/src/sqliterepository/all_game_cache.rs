...
impl AllGameCacheRepository for RepositoryImpl<domain::all_game_cache::AllGameCache> {
    ...
    async fn get_all(&mut self) -> anyhow::Result<AllGameCache> {
        let rows: Vec<AllGameCacheTable> = self
            .executor
            .with_conn(|conn| {
                Box::pin(async move {
                    let rows =
                        sqlx::query_as::<_, AllGameCacheTable>("select * from all_game_caches")
                            .fetch_all(conn)
                            .await?;
                    Ok(rows)
                })
    ...
    async fn get_last_updated(&mut self) -> anyhow::Result<(i32, DateTime<Local>)> {
        let (id, ts): (i32, sqlx::types::chrono::NaiveDateTime) = self
            .executor
            .with_conn(|conn| {
                Box::pin(async move {
                    let last_updated: (i32, sqlx::types::chrono::NaiveDateTime) =
                ...
                .with_conn(|conn| {
                    Box::pin(async move {
                        let mut qb = sqlx::query_builder::QueryBuilder::new(
                            "INSERT INTO all_game_caches (id, gamename, thumbnail_url) ",
                        );
                        qb.push_values(chunk, |mut b, new| {
                            b.push_bind(new.id);
    ...
    async fn delete_by_ids(&mut self, ids: Vec<i32>) -> anyhow::Result<()> {
        ...
        self.executor
            .with_conn(|conn| {
                Box::pin(async move {
                    let mut qb = sqlx::query_builder::QueryBuilder::new(
                        "DELETE FROM all_game_caches WHERE id IN (",
                    );
                    {
