...
pub fn process_square_icon(src: &str, dst: &str, target_short_px: u32) -> anyhow::Result<()> {
    ...
    let height =
        std::num::NonZeroU32::new(src_h).ok_or_else(|| anyhow::anyhow!("invalid height"))?;
    ...
    let dst_width =
        std::num::NonZeroU32::new(dst_w).ok_or_else(|| anyhow::anyhow!("invalid dst width"))?;
    let dst_height =
        std::num::NonZeroU32::new(dst_h).ok_or_else(|| anyhow::anyhow!("invalid dst height"))?;
    ...
    let cropped =
        image::imageops::crop_imm(&mut resized, x, y, target_short_px, target_short_px).to_image();
...
impl IconServiceImpl {
    ...
    pub fn new_from_root_path(_root_dir: String) -> Self {
        Self {
            backend: Backend::Host {
                resolver: Arc::new(DirsSavePathResolver::default()),
            },
        }
    }
    ...
    pub(crate) fn write_default_icon(save_path: &str) -> anyhow::Result<()> {
        let bytes = include_bytes!("../../../icons/notfound.png");
    ...
    async fn save_icon_from_url(
        ...
        match &self.backend {
            Backend::Tauri(_handle) => {
                ...
                match process_square_icon(
                    &orig.to_string_lossy(),
                    &save_path,
                    Self::ICON_TARGET_SHORT_PX,
                ) {
                    Ok(_) => Ok(()),
                    Err(e) => {
            ...
            Backend::Host { resolver } => {
                ...
                let filename = url::Url::parse(url)
                    ...
                    .unwrap_or_else(|| "icon".to_string());
                let orig = std::env::temp_dir()
                    .join("launcherg-images")
                    .join(format!("{}-{}", id.value, filename));
                thumb::download_to_file(url, &orig.to_string_lossy()).await?;
                match process_square_icon(
                    &orig.to_string_lossy(),
                    &save_path,
