use domain::windows::shell_link::{CreateShortcutRequest, ShellLink};
use windows::{
    ...
    Win32::{
        ...
        UI::Shell::{
            IShellLinkW, ShellExecuteExW, ShellLink as ShellLinkCom, SEE_MASK_NOCLOSEPROCESS,
            SHELLEXECUTEINFOW,
        },
...
impl ShellLink for ShellLinkImpl {
        ...
        unsafe {
            ...
            for item in items.iter() {
                let shell_link: IShellLinkW =
                    CoCreateInstance(&ShellLinkCom, None, CLSCTX_INPROC_SERVER)?;
                shell_link.SetPath(PCWSTR::from_raw(
                    (&item.target_path as &str)
                        .to_wide_null_terminated()
                        .as_ptr(),
                ))?;
                if let Some(wd) = &item.working_dir {
                    shell_link.SetWorkingDirectory(PCWSTR::from_raw(
                        (wd.as_str()).to_wide_null_terminated().as_ptr(),
                    ))?;
                }
                if let Some(args) = &item.arguments {
                    shell_link.SetArguments(PCWSTR::from_raw(
                        (args.as_str()).to_wide_null_terminated().as_ptr(),
                    ))?;
                }
                if let Some(icon) = &item.icon_path {
                    shell_link.SetIconLocation(
                        PCWSTR::from_raw((icon.as_str()).to_wide_null_terminated().as_ptr()),
                        0,
                    )?;
                }
    ...
    fn get_lnk_metadatas(
        &self,
        lnk_file_paths: Vec<String>,
    ) -> anyhow::Result<std::collections::HashMap<String, domain::file::LnkMetadata>> {
        let mut metadatas: std::collections::HashMap<String, domain::file::LnkMetadata> =
        ...
        unsafe {
            ...
            for file_path in lnk_file_paths.into_iter() {
                if file_path.to_lowercase().ends_with("lnk") {
                    ...
                    let icon_file: Option<String> =
                        domain::file::get_url_file_icon_path(&file_path)?;
                    metadatas.insert(
                        file_path.clone(),
                        domain::file::LnkMetadata {
                            path: file_path,
    ...
    fn execute_lnk<'a>(
        ...
        unsafe {
            ...
            if is_run_as_admin {
                sei.lpVerb = PCWSTR::from_raw((&"runas").to_wide_null_terminated().as_ptr());
            }
