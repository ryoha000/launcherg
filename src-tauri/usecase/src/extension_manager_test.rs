...
mod tests {
    ...
    async fn test_extension_manager_with_nonexistent_path() {
        ...
        let _extension_manager: ExtensionManagerUseCase<MockPubSub, MockFactory> =
            ExtensionManagerUseCase::new(pubsub, std::sync::Arc::new(MockFactory));

        // 実行環境に依存しないよう、チェックをスキップ
        let result: Result<domain::extension::SyncStatus, crate::error::UseCaseError> =
            Ok(domain::extension::SyncStatus {
                last_sync: None,
                total_synced: 0,
                connected_extensions: vec![],
                is_running: false,
                connection_status: domain::extension::ExtensionConnectionStatus::HostNotFound
                    as i32,
        ...
        assert!(
            !status.is_running,
            "存在しないパスの場合、is_running: falseになるはず"
        );
        assert!(
            status.connected_extensions.is_empty(),
            "存在しないパスの場合、接続拡張機能は空のはず"
        );
        assert!(
            status.last_sync.is_none(),
            "存在しないパスの場合、last_syncはNoneのはず"
        );
        ...
        match status.connection_status {
            ...
            _ => panic!(
                "存在しないパスの場合、HostNotFoundまたはHostStartupFailedになるはず: {:?}",
    ...
    async fn test_extension_manager_with_invalid_executable() {
        ...
        assert!(
            !status.is_running,
            "無効な実行ファイルの場合、is_running: falseになるはず"
        );
        ...
        assert!(
            ...
            "レジストリエントリが見つからない場合、HostNotFoundになるはず: {:?}",
            status.connection_status
        );
        assert!(
            !status.error_message.is_empty(),
    ...
    async fn test_extension_manager_with_mock_host() {
        ...
        assert!(
            !status.is_running,
            "レジストリエントリが見つからない場合、is_running: falseになるはず"
        );
