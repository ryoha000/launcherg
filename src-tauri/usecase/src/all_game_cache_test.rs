...
mod tests {
    ...
    use domain::{
        all_game_cache::{AllGameCacheOne, AllGameCacheOneWithThumbnailUrl, NewAllGameCacheOne},
    ...
    async fn test_get_success() {
        ...
        let use_case = AllGameCacheUseCase::new(Arc::new(
            crate::repositorymock::TestRepositoryManager::new(repos),
        ));
    ...
    async fn test_get_cache_last_updated_success() {
        ...
        let mock_repo = {
            ...
            repo.expect_get_last_updated().times(1).returning(move || {
                Box::pin(async move { Ok::<_, anyhow::Error>((100, expected_time)) })
            });
            repo
        };
