...
impl domain::repository::works::DlsiteWorkRepository for TestRepositories {
    async fn upsert(&mut self, new_work: &domain::works::NewDlsiteWork) -> anyhow::Result<domain::Id<domain::works::DlsiteWork>> { self.dlsite_work.lock().await.upsert(new_work).await }
    async fn find_by_store_key(&mut self, store_id: &str, category: &str) -> anyhow::Result<Option<domain::works::DlsiteWork>> { self.dlsite_work.lock().await.find_by_store_key(store_id, category).await }
}

#[cfg(test)]
impl domain::repository::dmm_work_pack::DmmPackRepository for TestRepositories {
    async fn add(&mut self, work_id: domain::Id<domain::works::Work>) -> anyhow::Result<()> { self.dmm_pack.lock().await.add(work_id).await }
    async fn remove(&mut self, work_id: domain::Id<domain::works::Work>) -> anyhow::Result<()> { self.dmm_pack.lock().await.remove(work_id).await }
    async fn list(&mut self) -> anyhow::Result<Vec<domain::dmm_work_pack::DmmWorkPack>> { self.dmm_pack.lock().await.list().await }
    async fn exists(&mut self, work_id: domain::Id<domain::works::Work>) -> anyhow::Result<bool> { self.dmm_pack.lock().await.exists(work_id).await }
}

#[cfg(test)]
impl domain::repository::all_game_cache::AllGameCacheRepository for TestRepositories {
    async fn get_by_ids(&mut self, ids: Vec<i32>) -> anyhow::Result<Vec<domain::all_game_cache::AllGameCacheOneWithThumbnailUrl>> { self.all_game_cache.lock().await.get_by_ids(ids).await }
    async fn get_all(&mut self) -> anyhow::Result<domain::all_game_cache::AllGameCache> { self.all_game_cache.lock().await.get_all().await }
    ...
    async fn update(&mut self, cache: Vec<domain::all_game_cache::NewAllGameCacheOne>) -> anyhow::Result<()> { self.all_game_cache.lock().await.update(cache).await }
    async fn search_by_name(&mut self, name: &str) -> anyhow::Result<Vec<domain::all_game_cache::AllGameCacheOneWithThumbnailUrl>> { self.all_game_cache.lock().await.search_by_name(name).await }
}

#[cfg(test)]
impl domain::repository::explored_cache::ExploredCacheRepository for TestRepositories {
    async fn get_all(&mut self) -> anyhow::Result<domain::explored_cache::ExploredCache> { self.explored_cache.lock().await.get_all().await }
    async fn add(&mut self, adding: domain::explored_cache::ExploredCache) -> anyhow::Result<()> { self.explored_cache.lock().await.add(adding).await }
}
...
impl domain::repository::collection::CollectionRepository for TestRepositories {
    ...
    async fn get_not_registered_info_element_ids(&mut self) -> anyhow::Result<Vec<domain::Id<domain::collection::CollectionElement>>> { self.collection.lock().await.get_not_registered_info_element_ids().await }
    async fn upsert_collection_element_paths(&mut self, paths: &domain::collection::NewCollectionElementPaths) -> anyhow::Result<()> { self.collection.lock().await.upsert_collection_element_paths(paths).await }
    async fn get_element_paths_by_element_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<Option<domain::collection::CollectionElementPaths>> { self.collection.lock().await.get_element_paths_by_element_id(id).await }
    async fn upsert_collection_element_install(&mut self, install: &domain::collection::NewCollectionElementInstall) -> anyhow::Result<()> { self.collection.lock().await.upsert_collection_element_install(install).await }
    async fn get_element_install_by_element_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<Option<domain::collection::CollectionElementInstall>> { self.collection.lock().await.get_element_install_by_element_id(id).await }
    async fn upsert_collection_element_play(&mut self, play: &domain::collection::NewCollectionElementPlay) -> anyhow::Result<()> { self.collection.lock().await.upsert_collection_element_play(play).await }
    async fn get_element_play_by_element_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<Option<domain::collection::CollectionElementPlay>> { self.collection.lock().await.get_element_play_by_element_id(id).await }
    async fn update_element_last_play_at_by_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>, last_play_at: chrono::DateTime<chrono::Local>) -> anyhow::Result<()> { self.collection.lock().await.update_element_last_play_at_by_id(id, last_play_at).await }
    async fn upsert_collection_element_like(&mut self, like: &domain::collection::NewCollectionElementLike) -> anyhow::Result<()> { self.collection.lock().await.upsert_collection_element_like(like).await }
    async fn delete_collection_element_like_by_element_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<()> { self.collection.lock().await.delete_collection_element_like_by_element_id(id).await }
    async fn get_element_like_by_element_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<Option<domain::collection::CollectionElementLike>> { self.collection.lock().await.get_element_like_by_element_id(id).await }
    async fn update_element_like_at_by_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>, like_at: Option<chrono::DateTime<chrono::Local>>) -> anyhow::Result<()> { self.collection.lock().await.update_element_like_at_by_id(id, like_at).await }
    async fn upsert_collection_element_thumbnail(&mut self, thumbnail: &domain::collection::NewCollectionElementThumbnail) -> anyhow::Result<()> { self.collection.lock().await.upsert_collection_element_thumbnail(thumbnail).await }
    async fn get_element_thumbnail_by_element_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<Option<domain::collection::CollectionElementThumbnail>> { self.collection.lock().await.get_element_thumbnail_by_element_id(id).await }
    async fn upsert_collection_element_thumbnail_size(&mut self, id: &domain::Id<domain::collection::CollectionElement>, width: i32, height: i32) -> anyhow::Result<()> { self.collection.lock().await.upsert_collection_element_thumbnail_size(id, width, height).await }
    async fn get_null_thumbnail_size_element_ids(&mut self) -> anyhow::Result<Vec<domain::Id<domain::collection::CollectionElement>>> { self.collection.lock().await.get_null_thumbnail_size_element_ids().await }
    async fn get_collection_id_by_erogamescape_id(&mut self, erogamescape_id: i32) -> anyhow::Result<Option<domain::Id<domain::collection::CollectionElement>>> { self.collection.lock().await.get_collection_id_by_erogamescape_id(erogamescape_id).await }
    async fn get_collection_ids_by_erogamescape_ids(&mut self, erogamescape_ids: &[i32]) -> anyhow::Result<Vec<(i32, domain::Id<domain::collection::CollectionElement>)>> { self.collection.lock().await.get_collection_ids_by_erogamescape_ids(erogamescape_ids).await }
    async fn get_collection_id_by_dmm_mapping(&mut self, store_id: &str, category: &str, subcategory: &str) -> anyhow::Result<Option<domain::Id<domain::collection::CollectionElement>>> { self.collection.lock().await.get_collection_id_by_dmm_mapping(store_id, category, subcategory).await }
    async fn get_collection_ids_by_dmm_mappings(&mut self, keys: &[(String, String, String)]) -> anyhow::Result<Vec<(String, String, String, domain::Id<domain::collection::CollectionElement>)>> { self.collection.lock().await.get_collection_ids_by_dmm_mappings(keys).await }
    async fn get_collection_ids_by_work_ids(&mut self, work_ids: &[i32]) -> anyhow::Result<Vec<(i32, domain::Id<domain::collection::CollectionElement>)>> { self.collection.lock().await.get_collection_ids_by_work_ids(work_ids).await }
    ...
    async fn upsert_work_mapping(&mut self, collection_element_id: &domain::Id<domain::collection::CollectionElement>, work_id: i32) -> anyhow::Result<()> { self.collection.lock().await.upsert_work_mapping(collection_element_id, work_id).await }
    async fn get_work_ids_by_collection_ids(&mut self, collection_element_ids: &[i32]) -> anyhow::Result<Vec<(domain::Id<domain::collection::CollectionElement>, i32)>> { self.collection.lock().await.get_work_ids_by_collection_ids(collection_element_ids).await }
    ...
    async fn allocate_new_collection_element_id(&mut self, gamename: &str) -> anyhow::Result<domain::Id<domain::collection::CollectionElement>> { self.collection.lock().await.allocate_new_collection_element_id(gamename).await }
    async fn get_erogamescape_id_by_collection_id(&mut self, id: &domain::Id<domain::collection::CollectionElement>) -> anyhow::Result<Option<i32>> { self.collection.lock().await.get_erogamescape_id_by_collection_id(id).await }
}
...
impl domain::repository::native_host_log::NativeHostLogRepository for TestRepositories {
    async fn insert_log(&mut self, level: domain::native_host_log::HostLogLevel, typ: domain::native_host_log::HostLogType, message: &str) -> anyhow::Result<()> { self.host_log.lock().await.insert_log(level, typ, message).await }
    async fn list_logs(&mut self, limit: i64, offset: i64, level: Option<domain::native_host_log::HostLogLevel>, typ: Option<domain::native_host_log::HostLogType>) -> anyhow::Result<Vec<domain::native_host_log::NativeHostLogRow>> { self.host_log.lock().await.list_logs(limit, offset, level, typ).await }
    async fn count_logs(&mut self, level: Option<domain::native_host_log::HostLogLevel>, typ: Option<domain::native_host_log::HostLogType>) -> anyhow::Result<i64> { self.host_log.lock().await.count_logs(level, typ).await }
}

#[cfg(test)]
impl domain::repository::work_omit::WorkOmitRepository for TestRepositories {
    async fn add(&mut self, work_id: domain::Id<domain::works::Work>) -> anyhow::Result<()> { self.work_omit.lock().await.add(work_id).await }
    async fn remove(&mut self, work_id: domain::Id<domain::works::Work>) -> anyhow::Result<()> { self.work_omit.lock().await.remove(work_id).await }
    async fn list(&mut self) -> anyhow::Result<Vec<domain::work_omit::WorkOmit>> { self.work_omit.lock().await.list().await }
    async fn exists(&mut self, work_id: domain::Id<domain::works::Work>) -> anyhow::Result<bool> { self.work_omit.lock().await.exists(work_id).await }
}
